...
func ChangeProduct(w http.ResponseWriter, r *http.Request, client product.ProductsServiceClient) {
	...
	var newProductGrpc = product.ReturnNewProduct{
		Id:                   int64(productId),
	...
	product_ids = append(product_ids, int64(productId))
	...
	oldImageUrl := resp.Products[0].ImageUrl

	data, err := client.ChangeProduct(ctx, &newProductGrpc)
	if err != nil {
		http.Error(w, "server error", http.StatusInternalServerError)
		if savedFilePath != "" {
